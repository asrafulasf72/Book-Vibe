...
const Nabvar = () => {
    ...
    return (
          <nav className="flex justify-between items-center max-w-[1140px] mx-auto my-5">
             <div>
                <span onClick={()=>setOpen(!Open)} className="flex gap-2 justify-center items-center">
                    {
                        Open?<X className="md:hidden"></X>:<Menu className="md:hidden"></Menu>
                    }
                    <div className={`md:hidden duration-1000 flex flex-col absolute ${Open?'top-16':'-top-40'} `}>
                      <NavLink to='/'>Home</NavLink>
                      <NavLink to='/books'>Listed Books</NavLink>
                      <NavLink to='/pages'>Pages to Read</NavLink>
                    </div>
                    <h1 className="text-[1.6rem] font-bold text-black work-sans">Book Vibe</h1>
                </span>
             </div>
